...
print(">>> EVALUATOR: About to import LanguagePostProcessor. Current sys.path[0]:", sys.path[0])
# This ensures we see which one is being loaded
import adet.utils.language_postprocessor
print(">>> EVALUATOR: adet.utils.language_postprocessor location:", adet.utils.language_postprocessor.__file__)
from adet.utils.language_postprocessor import LanguagePostProcessor
...
class TextEvaluator():
    ...
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        if "totaltext" in dataset_name:
            ...
            print("<<<<< TextEvaluator: Instantiating MINIMAL LanguagePostProcessor >>>>>")
            self.language_post_processor = LanguagePostProcessor(cfg)
            self._word_spotting = False
    ...
    def process(self, inputs, outputs):
        print("<<<<< TextEvaluator PROCESS method called >>>>>")
        for i, o in zip(inputs, outputs):
            prediction = {"image_id": i["image_id"]}
            if "instances" in o:
                instances = o["instances"].to(self._cpu_device) # Keep cpu device
                prediction["instances"] = self.instances_to_coco_json(instances, i)
            else:
    ...
    def instances_to_coco_json(self, instances, inputs):
        print("<<<<< instances_to_coco_json CALLED >>>>>")
        img_id = inputs["image_id"]
        width = inputs['width']
        height = inputs['height']
        if not instances: return []
        scores = instances.scores.tolist()
        if not hasattr(instances, 'char_logits'): return [] # Guard
        char_logits_all = instances.char_logits

        results = []
        for idx, score_val in enumerate(scores): # Renamed score to score_val
            print(f"<<<<< instances_to_coco_json: Processing instance {idx} >>>>>")
            current_instance_logits = char_logits_all[idx]

            print(f"  Calling MINIMAL language_post_processor.refine for instance {idx}")
            s = self.language_post_processor.refine(current_instance_logits)
            print(f"  Result from MINIMAL refine for instance {idx}: '{s}'")

            # Minimal result structure
            results.append({
                "image_id": img_id, "category_id": 1, "polys": [], "rec": s, "score": score_val
            })
        return results
        #---KY+ ---

    def pnt_to_polygon(self, ctrl_pnt): return [] # Dummy
